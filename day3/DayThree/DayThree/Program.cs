...
namespace DayThree
{
    ...
    {
        readonly IList<string> _wireOne;
        readonly IList<string> _wireTwo;
        readonly Byte[] _bitArray;
        readonly int _width;
        readonly int _height;
        readonly int _closest;
        readonly int _offsetX;
        readonly int _offsetY;
        readonly IDictionary<Tuple<int, int>, int> _stepCounts;
        readonly int _stepCount;

        public int Closest => _closest;

        public int StepCount => _stepCount;


        public CrossedWires(string csv1, string csv2, bool first) : this(csv1.Split(',').ToList(), csv2.Split(',').ToList(), first)
        {
            
        }
        public CrossedWires(IList<string> w1, IList<string> w2, bool first)
        {
            _wireOne = w1;
            _wireTwo = w2;
            Tuple<int, int, int, int> dimensions = Dimensions(w1,0,0,0,0);
            dimensions = Dimensions(w2, dimensions.Item1, dimensions.Item2, dimensions.Item3, dimensions.Item4);
            _width = (int)Math.Ceiling((decimal)(dimensions.Item2 - dimensions.Item1) / 8);
            _height = (dimensions.Item4 - dimensions.Item3) +2;
            _offsetX = dimensions.Item1;
            _offsetY = dimensions.Item3;
            _bitArray = new byte[_width * _height];
            _stepCounts = new Dictionary<Tuple<int, int>, int>();
            DoWire(true);
            var intersections = DoWire(false);
            _closest = intersections.Min();
            if (first)
            {
                var otherStepCounts = new CrossedWires(w2, w1, false)._stepCounts;
                _stepCount = SmallestStepCount(otherStepCounts);
            }
        }

        int SmallestStepCount(IDictionary<Tuple<int, int>, int> otherStepCounts)
        {
            int sum = 0;
            foreach(var kvp in _stepCounts)
            {
                int s = kvp.Value + otherStepCounts[kvp.Key];
                if (sum == 0 || s < sum)
                    sum = s;
            }
            return sum;
        ...
        IList<int> DoWire(bool IsWireOne)
        {
            int x = 0;
            int y = 0;
            int stepCount = 0;
            IList<int> rv = new List<int>();
            foreach (string move in IsWireOne ? _wireOne : _wireTwo)
            {
                ...
                for (int i = 0; i < distance; ++i)
                {
                    ...
                    else
                    {
                        if (Check(x, y))
                        {
                            rv.Add(Manhattan(x, y));
                            _stepCounts.Add(new Tuple<int, int>(x, y), stepCount);
                        }
                    }
                }
            }
        ...
        bool Check(int x, int y)
        {
            ...
            int offset = 7 - ((x-_offsetX) % 8);
            byte b = (byte)(1 << offset);
            return (_bitArray[index] & b) == b;
        ...
        int MapToIndex(int x, int y)
        {
            x -= _offsetX;
            y -= _offsetY;
            return ((int)Math.Floor((decimal)x / 8)) + (y * _width);
        }
        ...
        Tuple<int, int, int, int> Dimensions(IList<string> w, int minwidth, int maxwidth, int minheight, int maxheight)
        {
            int width=0;
            int height=0;
            foreach(string move in w)
            {
                int distance = Convert.ToInt32(move.Substring(1));
                switch (move[0]) {
                    case ('U'):
            ...
            return new Tuple<int, int,int,int>(minwidth-1, maxwidth+1, minheight-1, maxheight+1); // just to be sure :) 
        }
